...
const COHORT_PATH = process.env.CERT_COHORT_PATH ?? '/var/tmp/p1b-cohort.json'
...
async function main(): Promise<void> {
  ...
  if (cohortIds.length !== 35) throw new Error(`expected 35 cohort ids, got ${cohortIds.length}`)
  ...
  const assignment: Record<string, ManifestRow[]> = {
    w1: batchRows.slice(0, 5),
    w2: batchRows.slice(5, 15),
    w3: batchRows.slice(15, 25),
    w4: batchRows.slice(25, 35),
  }
  ...
  const runId = `physicsP1b-4w-${Date.now()}`
  const startedAt = new Date().toISOString()
  mkdirSync(ARTIFACT_DIR, { recursive: true })
  const artifactPath = join(ARTIFACT_DIR, `${runId}.jsonl`)

  process.stderr.write(`Physics P-1b (4-worker, all remaining UNMEASURED) ${runId}\n`)
