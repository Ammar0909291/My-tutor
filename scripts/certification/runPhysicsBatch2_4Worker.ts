/**
 * TIER A — PHYSICS BATCH 2, 4-WORKER (25 concepts, bias away from w1).
 *
 * Same pattern as runPhysicsCanary4Worker.ts (which passed cleanly): a
 * thin, one-off dispatch script that imports runTierA.ts's already-exported
 ...
 * any other part of the frozen certification instrument). Only the concept
 * selection and worker split differ from the standing 2-per-subject scheme,
 * and only for this one batch.
 *
 * CONCEPT SELECTION: physics manifest rows 76-100 (0-indexed 75-99),
 * lessonOrder-sorted. Verified beforehand to have ZERO overlap with the 75
 * physics concepts already attempted (batch 1: rows 1-25 + 120-144; canary:
 * rows 51-75).
 *
 * WORKER SPLIT: 4 / 7 / 7 / 7 (w1/w2/w3/w4) = 25. Biased away from w1 per
 * the canary's own finding (w1's replay cost is ~61x a fresh account's,
 * driven by its 30k+ event lifetime history) — w2/w3/w4 do proportionally
 * more of this batch's concept count while w1 still participates
 * concurrently, exactly as instructed.
 *
 *   npx tsx scripts/certification/runPhysicsBatch2_4Worker.ts
 */
...
async function main(): Promise<void> {
  ...
  // Global rows 76-100 (0-indexed 75..99) — the next unclaimed slice after
  // batch 1 (rows 1-25, 120-144) and the canary (rows 51-75).
  const batchRows = physics.slice(75, 100)
  ...
  const runId = `physicsBatch2-4w-${Date.now()}`
  const startedAt = new Date().toISOString()
  mkdirSync(ARTIFACT_DIR, { recursive: true })
  const artifactPath = join(ARTIFACT_DIR, `${runId}.jsonl`)

  process.stderr.write(`Physics batch 2 (4-worker) ${runId}\n`)
