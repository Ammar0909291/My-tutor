...
const COHORT_PATH = process.env.CERT_COHORT_PATH ?? '/var/tmp/batch6-cohort.json'
...
async function main(): Promise<void> {
  ...
  const runId = `physicsBatch6-4w-${Date.now()}`
  const startedAt = new Date().toISOString()
  mkdirSync(ARTIFACT_DIR, { recursive: true })
  const artifactPath = join(ARTIFACT_DIR, `${runId}.jsonl`)

  process.stderr.write(`Physics P-1 (4-worker, post-R3 UNMEASURED cohort) ${runId}\n`)
