...
// Restrict to ENGLISH only -- P-10 collision analysis is meaningless mixed
// with other subjects' pre-existing ladder promotions.
...
const ENGLISH_BEFORE = [
  ...
  ...ENGLISH_PROBE_BATCH_9, ...ENGLISH_PROBE_BATCH_10,
]
const ENGLISH_AFTER = [...ENGLISH_BEFORE, ...ENGLISH_ADULT_BAND_BATCH_1]
...
console.log(`Batch size: ${ENGLISH_ADULT_BAND_BATCH_1.length} (expect 30)`)
check(ENGLISH_ADULT_BAND_BATCH_1.length === 30, 'batch size')

// 1. exactly one correct choice per probe, ADULT band, >=2 choices
for (const p of ENGLISH_ADULT_BAND_BATCH_1) {
...
// 2. every wrong choice carries a misconceptionId, and it is REUSED (not invented) --
// i.e. it already appears somewhere in the pre-existing English corpus.
...
// 3. no exact-duplicate stems within the batch, and no stem exactly matches a
// pre-existing English probe's stem (would be a real question repeat)
const stems = new Map<string, number>()
for (const p of ENGLISH_ADULT_BAND_BATCH_1) stems.set(p.stem, (stems.get(p.stem) ?? 0) + 1)
...
// 4. answer-length giveaway check
for (const p of ENGLISH_ADULT_BAND_BATCH_1) {
...
// 5. P-10 check: adding this batch must not re-identify any PRE-EXISTING
// (before-batch) probe's slug -- compare the abandoned-slug set before vs after.
...
// 6. slug uniqueness WITHIN the full English-after corpus (no collisions)
