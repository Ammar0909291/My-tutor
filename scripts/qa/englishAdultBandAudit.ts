...
export function computeAdultGap(): { adultGap: string[]; adultPartial: string[]; adultOk: string[]; kgIds: string[] } {
  ...
  return { adultGap, adultPartial, adultOk, kgIds }
}

function main(): void {
  const { adultGap, adultPartial, adultOk, kgIds } = computeAdultGap()
  ...
  console.log('\nFirst 60 ADULT-gap concepts, KG order:')
  adultGap.slice(0, 60).forEach((id, i) => console.log(`${String(i + 1).padStart(3)}. ${id}`))
  if (adultPartial.length) {
    console.log('\nPartial (1-2 probes), KG order:')
    adultPartial.forEach((id, i) => console.log(`${String(i + 1).padStart(3)}. ${id}`))
  }
}
