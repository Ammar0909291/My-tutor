/**
 * Real-learner QA driver for the English ADULT-band probe-contract Batch 11
 * (10 concepts: 4 listening + 6 speaking, see englishAdultBandBatch11.ts).
 *
 * Same persona and technique as Batches 1-10's live-QA drivers. Deliberately
 ...
 * positive found in Groups 3-10's QA
 * (docs/architecture/ENGLISH_MCQ_REOFFER_FALSE_POSITIVE_FINDING.md).
 *
 * Run: QA_PASSWORD=... npx tsx scripts/qa/englishAdultBandBatch11LiveQa.ts
 */
...
async function run(): Promise<void> {
  ...
  // Lesson A: eng.speaking.conversation-skills -- nuanced (talking a lot
  // isn't good conversation, silence isn't always bad), useful adult skill.
  await runLesson(cookie, 'english', 'eng.speaking.conversation-skills', [
    { msg: "hi, conversation skills is new for me to think about" },
    { msg: "so a good conversation means i should talk as much as possible right?" },
    { msg: "i dont understand why silence would ever be ok in a conversation" },
  ...
  // Lesson B: eng.listening.critical-listening -- nuanced (confident
  // delivery isn't a stronger argument, any evidence isn't equal), useful
  // adult skill.
  await runLesson(cookie, 'english', 'eng.listening.critical-listening', [
    { msg: "hi, critical listening sounds tricky for me" },
    { msg: "so a confident speaker is always making the stronger argument right?" },
    { msg: "i dont understand why any evidence someone mentions doesnt count equally" },
