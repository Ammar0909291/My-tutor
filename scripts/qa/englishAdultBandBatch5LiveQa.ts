/**
 * Real-learner QA driver for the English ADULT-band probe-contract Batch 5
 * (FINAL batch — 10 concepts: all core grammar, see englishAdultBandBatch5.ts).
 *
 * Same persona and technique as Batches 1-4's live-QA drivers. Deliberately
 ...
 * positive found in Groups 3-4's QA
 * (docs/architecture/ENGLISH_MCQ_REOFFER_FALSE_POSITIVE_FINDING.md).
 *
 * Run: QA_PASSWORD=... npx tsx scripts/qa/englishAdultBandBatch5LiveQa.ts
 */
...
async function run(): Promise<void> {
  ...
  // Lesson A: eng.grammar.past-tenses -- nuanced (irregular verbs vs. the
  // -ed rule, past continuous vs simple past), genuinely difficult for a
  // weak-English learner.
  await runLesson(cookie, 'english', 'eng.grammar.past-tenses', [
    { msg: "hi, past tense is confusing for me, please go slow" },
    { msg: "so every past tense verb just adds ed right?" },
    { msg: "i dont understand irregular verbs, can you explain" },
    { msg: "can you try explaining it differently" },
    { msg: "i dont know", pickWrong: true },
    { msg: "oh wait, i think i misread that" },
    { msg: "ok trying again", pickCorrect: true },
    { msg: "thanks, that makes sense now" },
    { msg: "can you show me a diagram for this" },
    { msg: "alright, next question please", pickCorrect: true },
  ])

  // Lesson B: eng.grammar.complex-sentences -- nuanced (dependent vs.
  // independent clauses, subordinating conjunctions), a genuinely hard
  // concept for a weak learner and this campaign's final concept.
  await runLesson(cookie, 'english', 'eng.grammar.complex-sentences', [
    { msg: "hi, complex sentences sound hard, what are they" },
    { msg: "so its just any sentence with a conjunction in it right?" },
    { msg: "i dont get the difference from compound sentences" },
    { msg: "please explain it another way" },
    { msg: "i dont know", pickWrong: true },
    { msg: "hold on, let me reconsider" },
    { msg: "got it now", pickCorrect: true },
  ])

  console.log('\n' + '='.repeat(70))
  console.log('END OF QA RUN — FINAL GROUP')
  console.log('='.repeat(70))
}
