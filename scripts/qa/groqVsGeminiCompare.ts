/**
 * Provider comparison — real deployed app, one concept each in physics,
 * chemistry, english, driven twice: once forced to Groq, once forced to
 * Gemini, via the existing `x-cert-provider` header mechanism
 * (src/app/api/learn/chat/route.ts ~L196-219). That header is INERT unless
 * the authenticated user's own DB row has `modelOverrideAllowed = true` —
 * verified directly against production before writing this script
 * (suaibamr1@gmail.com already has it set to true, so no DB write was
 * needed to run this).
 *
 ...
 *
 * Run: npx tsx scripts/qa/groqVsGeminiCompare.ts
 */
...
async function runConcept(cookie: string, subject: string, lesson: CurriculumLesson, totalLessons: number, provider: 'groq' | 'gemini') {
  ...
  const providerHeader = { 'x-cert-provider': provider }
