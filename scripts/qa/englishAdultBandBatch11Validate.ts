...
const ENGLISH_AFTER = [...ENGLISH_BEFORE, ...ENGLISH_ADULT_BAND_BATCH_11]
...
console.log(`Batch size: ${ENGLISH_ADULT_BAND_BATCH_11.length} (expect 30)`)
check(ENGLISH_ADULT_BAND_BATCH_11.length === 30, 'batch size')

for (const p of ENGLISH_ADULT_BAND_BATCH_11) {
...
for (const p of ENGLISH_ADULT_BAND_BATCH_11) stems.set(p.stem, (stems.get(p.stem) ?? 0) + 1)
