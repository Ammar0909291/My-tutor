...
const ENGLISH_BEFORE = [
  ...
  ...ENGLISH_ADULT_BAND_BATCH_5, ...ENGLISH_ADULT_BAND_BATCH_6,
]
const ENGLISH_AFTER = [...ENGLISH_BEFORE, ...ENGLISH_ADULT_BAND_BATCH_7]
...
console.log(`Batch size: ${ENGLISH_ADULT_BAND_BATCH_7.length} (expect 30)`)
check(ENGLISH_ADULT_BAND_BATCH_7.length === 30, 'batch size')

for (const p of ENGLISH_ADULT_BAND_BATCH_7) {
...
for (const p of ENGLISH_ADULT_BAND_BATCH_7) stems.set(p.stem, (stems.get(p.stem) ?? 0) + 1)
