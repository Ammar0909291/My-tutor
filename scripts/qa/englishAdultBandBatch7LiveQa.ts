/**
 * Real-learner QA driver for the English ADULT-band probe-contract Batch 7
 * (10 concepts: 6 punctuation + 4 reading, see englishAdultBandBatch7.ts).
 *
 * Same persona and technique as Batches 1-6's live-QA drivers. Deliberately
 ...
 * positive found in Groups 3-6's QA
 * (docs/architecture/ENGLISH_MCQ_REOFFER_FALSE_POSITIVE_FINDING.md).
 *
 * Run: QA_PASSWORD=... npx tsx scripts/qa/englishAdultBandBatch7LiveQa.ts
 */
...
async function run(): Promise<void> {
  ...
  // Lesson A: eng.grammar.apostrophes -- nuanced (plural vs. possessive,
  // singular vs. plural possessive placement), classic ESL/weak-writer
  // difficulty.
  await runLesson(cookie, 'english', 'eng.grammar.apostrophes', [
    { msg: "hi, apostrophes confuse me a lot, please go slow" },
    { msg: "so i just add an apostrophe before s whenever a word ends in s right?" },
    { msg: "i dont understand plural possessive, like for multiple people" },
  ...
  // Lesson B: eng.reading.main-idea-and-details -- nuanced (main idea isn't
  // always the first sentence, can be implied), a genuinely useful reading
  // skill for a weak learner.
  await runLesson(cookie, 'english', 'eng.reading.main-idea-and-details', [
    { msg: "hi, finding the main idea is hard for me" },
    { msg: "so the main idea is always the first sentence right?" },
    { msg: "i dont understand how a main idea can be implied" },
