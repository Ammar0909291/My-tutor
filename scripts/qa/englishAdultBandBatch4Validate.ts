...
const ENGLISH_BEFORE = [
  ...
  ...ENGLISH_ADULT_BAND_BATCH_2, ...ENGLISH_ADULT_BAND_BATCH_3,
]
const ENGLISH_AFTER = [...ENGLISH_BEFORE, ...ENGLISH_ADULT_BAND_BATCH_4]
...
console.log(`Batch size: ${ENGLISH_ADULT_BAND_BATCH_4.length} (expect 30)`)
check(ENGLISH_ADULT_BAND_BATCH_4.length === 30, 'batch size')

for (const p of ENGLISH_ADULT_BAND_BATCH_4) {
...
for (const p of ENGLISH_ADULT_BAND_BATCH_4) stems.set(p.stem, (stems.get(p.stem) ?? 0) + 1)
