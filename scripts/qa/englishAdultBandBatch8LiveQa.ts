/**
 * Real-learner QA driver for the English ADULT-band probe-contract Batch 8
 * (10 concepts: all eng.reading, see englishAdultBandBatch8.ts).
 *
 * Same persona and technique as Batches 1-7's live-QA drivers. Deliberately
 ...
 * positive found in Groups 3-7's QA
 * (docs/architecture/ENGLISH_MCQ_REOFFER_FALSE_POSITIVE_FINDING.md).
 *
 * Run: QA_PASSWORD=... npx tsx scripts/qa/englishAdultBandBatch8LiveQa.ts
 */
...
async function run(): Promise<void> {
  ...
  // Lesson A: eng.reading.inference-in-reading -- nuanced (inference vs.
  // baseless guessing, explicit vs. implied information), genuinely
  // difficult reading skill for a weak learner.
  await runLesson(cookie, 'english', 'eng.reading.inference-in-reading', [
    { msg: "hi, making inferences from reading is hard for me" },
    { msg: "so if something isnt directly stated i cant know it right?" },
    { msg: "i dont understand the difference between inference and just guessing" },
    { msg: "can you explain it a different way" },
    { msg: "i dont know", pickWrong: true },
    { msg: "oh wait, i think i need to rethink that" },
  ...
  // Lesson B: eng.reading.critical-reading -- nuanced (critical reading
  // isn't just disagreeing, citing sources doesn't automatically mean
  // credible), a genuinely useful adult reading skill.
  await runLesson(cookie, 'english', 'eng.reading.critical-reading', [
    { msg: "hi, critical reading sounds intimidating to me" },
    { msg: "so critical reading just means finding fault with the text right?" },
    { msg: "i dont understand why citing statistics doesnt make something credible" },
