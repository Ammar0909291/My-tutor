/**
 * Real-learner QA driver for the English ADULT-band probe-contract Batch 10
 * (10 concepts: 6 writing + 4 listening, see englishAdultBandBatch10.ts).
 *
 * Same persona and technique as Batches 1-9's live-QA drivers. Deliberately
 ...
 * positive found in Groups 3-9's QA
 * (docs/architecture/ENGLISH_MCQ_REOFFER_FALSE_POSITIVE_FINDING.md).
 *
 * Run: QA_PASSWORD=... npx tsx scripts/qa/englishAdultBandBatch10LiveQa.ts
 */
...
async function run(): Promise<void> {
  ...
  // Lesson A: eng.listening.active-listening -- nuanced (hearing vs.
  // listening, active listening isn't silence), useful adult skill.
  await runLesson(cookie, 'english', 'eng.listening.active-listening', [
    { msg: "hi, active listening is a new idea for me" },
    { msg: "so hearing someone and listening to them is the same thing right?" },
    { msg: "i dont understand what makes listening active instead of just staying quiet" },
  ...
  // Lesson B: eng.writing.editing-and-proofreading -- nuanced (spell-check
  // doesn't catch everything, one silent read isn't enough), useful adult
  // writing skill.
  await runLesson(cookie, 'english', 'eng.writing.editing-and-proofreading', [
    { msg: "hi, editing and proofreading confuses me" },
    { msg: "so spell check catches every mistake right?" },
    { msg: "i dont understand why reading it once silently isnt enough" },
