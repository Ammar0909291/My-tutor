/**
 * Real-learner QA driver for the English ADULT-band probe-contract Batch 9
 * (10 concepts: 1 reading + 9 writing, see englishAdultBandBatch9.ts).
 *
 * Same persona and technique as Batches 1-8's live-QA drivers. Deliberately
 ...
 * positive found in Groups 3-8's QA
 * (docs/architecture/ENGLISH_MCQ_REOFFER_FALSE_POSITIVE_FINDING.md).
 *
 * Run: QA_PASSWORD=... npx tsx scripts/qa/englishAdultBandBatch9LiveQa.ts
 */
...
async function run(): Promise<void> {
  ...
  // Lesson A: eng.writing.topic-sentences -- nuanced (announcing vs.
  // claiming, specific vs. vague), a genuinely useful adult writing skill.
  await runLesson(cookie, 'english', 'eng.writing.topic-sentences', [
    { msg: "hi, topic sentences are confusing for me, please go slow" },
    { msg: "so a topic sentence just needs to announce the topic right?" },
    { msg: "i dont understand why it needs to be specific instead of general" },
  ...
  // Lesson B: eng.writing.narrative-writing -- nuanced (not just listing
  // events, dialogue isn't decoration), useful adult writing skill.
  await runLesson(cookie, 'english', 'eng.writing.narrative-writing', [
    { msg: "hi, narrative writing sounds hard for me" },
    { msg: "so a good narrative just lists everything that happened in order right?" },
    { msg: "i dont understand why dialogue matters if i can just summarize it" },
