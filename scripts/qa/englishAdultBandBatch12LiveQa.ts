/**
 * Real-learner QA driver for the English ADULT-band probe-contract Batch 12
 * (9 concepts: 2 speaking + 6 literature + 1 communication, see
 * englishAdultBandBatch12.ts). FINAL BATCH of the campaign.
 *
 * Same persona and technique as Batches 1-11's live-QA drivers. Deliberately
 ...
 * is already reported and tracked separately.
 *
 * Also watches (but does not fix) the MCQ re-offer disambiguation false-
 * positive found in Groups 3-11's QA
 * (docs/architecture/ENGLISH_MCQ_REOFFER_FALSE_POSITIVE_FINDING.md).
 *
 * Run: QA_PASSWORD=... npx tsx scripts/qa/englishAdultBandBatch12LiveQa.ts
 */
...
async function run(): Promise<void> {
  ...
  // Lesson A: eng.literature.point-of-view -- nuanced (first-person narrators
  // aren't automatically the full truth, omniscient/limited are genuinely
  // different), useful adult skill (reading contracts/reports critically).
  await runLesson(cookie, 'english', 'eng.literature.point-of-view', [
    { msg: "hi, point of view is confusing to me" },
    { msg: "so if a story is told in first person, its always the full truth right?" },
    { msg: "i dont understand how omniscient and limited third person are different" },
    { msg: "can you try a different way to explain it" },
    { msg: "i dont know", pickWrong: true },
    { msg: "oh wait, i think i see my mistake" },
    { msg: "ok let me try again", pickCorrect: true },
    { msg: "thanks that makes sense" },
    { msg: "can you show me a diagram for this" },
    { msg: "alright next question", pickCorrect: true },
  ])

  // Lesson B: eng.communication.digital-communication -- nuanced (same
  // register doesn't work everywhere, tone is hard not easy to convey in
  // text), directly useful adult workplace skill.
  await runLesson(cookie, 'english', 'eng.communication.digital-communication', [
    { msg: "hi, digital communication seems easy but maybe im wrong" },
    { msg: "so i can just use the same writing style for email and chat right?" },
    { msg: "i dont understand why tone would be hard to get across in a text message" },
