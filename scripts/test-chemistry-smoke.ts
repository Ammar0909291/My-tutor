/**
 * Phase 4: Chemistry Teaching Engine smoke test — through generic adapter
 * Run: npx tsx scripts/test-chemistry-smoke.ts
 */
import fs from 'fs'
...
const raw = JSON.parse(fs.readFileSync(path.join(ROOT, 'docs/chemistry/kg/graph.json'), 'utf-8'))
...
// Zod schema — identical to /api/teaching-engine route.ts
...
const IDS = [
  'chem.found.matter',           // foundational / understand
  'chem.atomic.electronic-config', // developing / apply
  'chem.bond.mo-theory',         // advanced    / analyze
  'chem.equil.buffer',           // proficient  / apply
  'chem.elect.nernst',           // proficient  / apply
...
console.log('\n═══════════════════════════════════════════════════════════')
console.log('PHASE 4 — CHEMISTRY TEACHING ENGINE SMOKE TEST (10 concepts)')
...
console.log(`\nPhase 4 Chemistry: ${pass ? 'ALL PASS ✓' : 'FAIL ✗'}`)
