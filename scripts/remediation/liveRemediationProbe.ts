/**
 * LIVE REMEDIATION-CARD PROBE — the harness that found six real defects
 * during the physics campaign (2026-08-27).
 *
 * Confusion-only script, no MCQ auto-answer: the third turn is a genuine
 * remediation request, which is the only kind of turn a remediation card can
 * own. Records what the deployed app actually does; does not judge.
 *
 * Usage:
 *   QA_EMAIL=... QA_PASSWORD=... SUBJECT=chemistry SLUG=chem.found.mole-concept \
 *     npx tsx scripts/remediation/liveRemediationProbe.ts
 *
 * SUBJECT defaults to physics; SLUG has no default (required).
 */
...
async function main() {
  ...
  console.log(`lesson ${les.order} "${les.lessonTitle}"  session ${sid}\n`)
  ...
  // NO MCQ AUTO-ANSWER. A remediation card can only own a remediation turn, so
  // the script must actually produce one.
  const script = [
    'sir i not understand this',       // CONFUSION      -> deterministic card
    'ok sir',                          // acknowledgement -> card does not own it
    'sir i still not understand can you explain again', // CONFUSION -> constrained source
  ]
  for (let n = 0; n < script.length; n += 1) {
    const p = await post('/api/learn/chat', { sessionId: sid, message: script[n] }, cookie)
    show(`T${n + 1}`, p, JSON.stringify(script[n]))
    last = p
  }
