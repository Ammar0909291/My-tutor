...
**REVOKED (explicit owner instruction, after 2026-09-06's real-account validation session):** `FORBIDDEN_ACCOUNTS = ['suaibamr@gmail.com']` no longer exists. It was previously defined identically in three places (`scripts/math/certify.ts`, `scripts/qa/liveAccount.ts`, and `scripts/certification/measurementIdentity.ts` as `PROTECTED_ACCOUNTS`/`isProtectedAccount`, with a per-worker `CERT_WORKER_<n>_DESIGNATED_TEST_ACCOUNT=true` opt-in override in the last of those) and enforced by throwing if a session's email matched. The owner has since directed that this account be available like any other account throughout My Tutor, including for certification and QA harnesses, and all three checks (plus the now-pointless override mechanism) have been removed accordingly. The disposable-account pattern (register `qa-<label>-<timestamp>@mytutor-qa.invalid`, drive it, call `DELETE /api/user/delete-account`, then independently confirm deletion by attempting and expecting failure of a re-login) is unaffected — it never depended on the forbidden-account check — and remains documented in `liveAccount.ts`'s own header as policy: *"a delete that is merely requested is not a delete that happened."*
