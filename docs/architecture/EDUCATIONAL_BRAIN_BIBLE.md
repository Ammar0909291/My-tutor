...
**ADR 15 finding (Rendered Reality Model):** ADR 12 addresses visual
*selection, generation, and caching* but not cross-turn *visual state
feedback*. The LLM has zero awareness of what visual is currently rendered
on the learner's screen: `VISUAL:` tags are stripped from stored messages,
`contextSnapshot` persists no visual state, and the `alreadyShown`
heuristic uses a phase-counter proxy, not rendered history. Consequence:
the LLM confabulates about what EXISTS on screen — a perception-chain
failure orthogonal to the evidence-chain failures addressed by Signal
Verification. **PROPOSED, ADR 15:** Rendered Reality Model — a
server-authoritative, per-session append-only log of rendered visual state
(identity + semantic description + source pipeline + concept + turn),
persisted in contextSnapshot, injected into the system prompt as a
RENDERED REALITY block. Five invariants, four-phase incremental migration.
Complements ADR 12 (ADR 12 = WHAT to show; ADR 15 = LLM KNOWS what was
shown).
...
| R16 | `generateVisualizationCode.ts` makes a second LLM call per turn when `ENABLE_DYNAMIC_VISUALIZATION` is true — a Permanent Rule 9 violation (the AI Router must be the only probabilistic component). **Correction 2026-07-21**: this doc previously said "the flag is off by default" — that was stale/wrong. `.env.example` ships `ENABLE_DYNAMIC_VISUALIZATION=true` as a deliberate, reasoned decision (confirmed via `git log -p .env.example`: "ON by default — flip OFF if you don't want the dynamic engine to run"), so the risk is REALIZED by default in the documented setup path (`cp .env.example .env`), not merely latent. It fires only when the deterministic VisualSpec/SceneSpec pipelines produce nothing for the turn, and results are cached per-concept globally, so it is not a violation on every turn — but it is a live, default-on second probabilistic call, not a dormant one | Open, flag-gated, **on by default** | ADR 12 §2, route.ts:17+1597, `.env.example:130` | PROPOSED in ADR 12 §4.4: move all visual LLM calls to background authoring tasks; per-turn path serves only cached `VisualAsset` records after first-time authoring; blocked on KG v1 freeze |
...
| R21 | The LLM has zero cross-turn awareness of rendered visual state — `VISUAL:` tags stripped from stored messages, `contextSnapshot` persists no visual state, `alreadyShown` is a phase-counter heuristic not rendered history. The LLM confabulates about what EXISTS on the learner's screen: a perception-chain failure class (tutor describes absent objects, ignores accurate learner observations, asks questions about nonexistent visuals, triggers recovery loops from its own confusion) orthogonal to the evidence-chain failures addressed by Signal Verification | **Already realized in production** | ADR 15; `ARCHITECTURAL_ROOT_CAUSE_ANALYSIS.md` | PROPOSED Rendered Reality Model (ADR 15) — server-authoritative visual-state log persisted in contextSnapshot, injected into system prompt. Four-phase incremental migration (persist-only → typed → semantic → learner observation cross-check) |
...
| R11 | ~~`nextBestAction.ts` carries three confirmed-dead exports that will never be removed~~ | **RESOLVED 2026-07-21** — the entire file (including `getChapterNextStep`, which ADR 04 recorded as the one genuinely-live export) was deleted as part of the School Mode dead-code removal. Verified via `git show` on the pre-deletion commit that `getChapterNextStep`'s only remaining caller was inside the already-dead `if(schoolCtx)` block in `route.ts` — School Mode's own route/UI (its real caller, the "chapter workspace page") had already been removed in commit `915d590`, predating ADR 04. Confirmed via full tsc + vitest suite: zero regressions | Finding 4, ADR 04 (superseded); School Mode removal commit `6a1ca0f` | Resolved by deletion — no residual risk. ADR 04's "permanently unexecuted by explicit user instruction" applied to a narrower, already-superseded scope (the file as it existed before `915d590` stubbed it); the file no longer exists |
...
| ADR 09 | Dynamic Lesson Composition (roadmap 4/8) | **Proposal, blocked on KG v1 freeze. EXTENDED BY PHASE 1 TQ-2 (not superseded).** | `composeLessonPlan()` has zero persisted cross-turn stage continuity; proposes generalizing the proven Worked Examples tag-emit/parse/persist/resume pattern via `contextSnapshot.lessonStageProgress` + a `planSignature` continuation/replan fingerprint. **Scope note (2026-07-31):** Phase 1's TQ-2 Pedagogical Planner *is* this ADR's selected Option B generalized — same snapshot key, same `planSignature`, same named `replan` event — extended cross-session and instantiated per strategy archetype (Phase 1 §5.7, RC-1…RC-5). ADR 09's Option C remains deferred on its own reasoning. Implementing Option B is Phase 1 Stage 5. |
...
| **Phase 1** | **Teaching Quality Architecture** (`PHASE_01_TEACHING_QUALITY_ARCHITECTURE.md`) | **CANONICAL** (v1.2.0, 2026-07-31) — repository-owner authorization following an independent merge-gate review performed outside this repository; no review artifact is committed here (its §15.1 is the approval record) | Introduces the **Campaign scale** — the per-concept, multi-turn, often multi-session layer between the session planner and the turn kernel — and seven components: TQ-1 Teaching Strategy Engine (9 archetypes, reconciled as the cross-concept generalization of the existing per-concept **Protocol** class), TQ-2 Pedagogical Planner (the 9-phase concept lesson arc; extends ADR 09), TQ-3 Teaching Method Library (~17 methods composing the 27 Actions, which are themselves named compounds over the ~91 Primitives), TQ-4 Adaptive Re-teaching (the Difference Operator: one declared primary axis plus a published dependency closure over 8 axes — makes paraphrase structurally illegal while keeping every escalation-ladder rung legal), TQ-5 Lesson Gold Standard (11 hard gates + 10 scored dimensions, each classified structurally-enforced vs measured), TQ-6 Teacher Decision Flow (the 8 teacher decisions resolved into `C-28`'s existing bands), TQ-7 Teaching Quality Metrics (process quality, bounded against OSF which retains outcome measurement). Adds **three narrowing-only constraint sources to `C-28` Band 2 and zero new decision authorities**; `C-29` remains sole authority on advancement. Publishes the canonical teaching hierarchy (§0.2) and a binding Architecture Reconciliation Procedure + self-certification prohibition (§17, §18) for all future phases. **Architecture only — canonicalization authorizes no implementation; all stages remain G1/G2 gated.** |
| **Phase 2** | **Visual Intelligence Architecture** (`PHASE_02_VISUAL_INTELLIGENCE_ARCHITECTURE.md`) | **CANONICAL** (v2.1.0, 2026-07-31) — repository-owner authorization; no review artifact committed (its §18.1 is the approval record) | The pedagogical visual tier, extending Phase 1. Answers *whether* a visual appears (VD-1: a four-condition admission test plus **ten enumerated contraindications**, default no), *why* (VD-2: a closed ten-purpose taxonomy — STRUCTURE-REVEAL, PROCESS-TRACE, RELATION-MAP, QUANTITY-SENSE, CONTRAST-DISCRIMINATE, MODEL-EXTERNALIZE, ATTENTION-DIRECT, MEMORY-OFFLOAD, LEARNER-CONSTRUCT, MANIPULATE-DISCOVER — each grounded in a distinct primitive composition, each carrying a one-sentence **claim**; a visual whose claim cannot be written is not admitted), and *which* form class (VD-3). Consumes the production tier by reference: **ADR 12** owns renderers, cache, validators and the Visual Policy; **ADR 14** owns the asset lifecycle Phase 2 targets; **ADR 15** owns rendered state. Two one-directional projections cross the tier boundary — `VisualIntent` down, `VisualAvailability` up — and neither is a call, so ADR 12 §13's leaf rule is preserved. Also: VD-6 makes **representation-dependence** a detectable state with withdrawal probes and three mastery gates (no gate passed with a supporting visual present; simulation success re-verified outside the sim; Phase 2 never certifies mastery — `C-29` does); VD-7 a nine-rung fallback ladder built for scarcity; VD-8 accessibility strengthened to instructional equivalence. **Architecture only — authorizes no implementation; W4-2 remains gated.** |
| **Phase 3** | **Adaptive Teaching Architecture** (`PHASE_03_ADAPTIVE_TEACHING_ARCHITECTURE.md`) | **CANONICAL** (v3.1.0, 2026-07-31) — repository-owner authorization; no review artifact committed (its §26.1 is the approval record) | The adaptive control plane, extending Phases 1 and 2. Fourteen components AT-1…AT-14 over six dials plus D3: the Adaptive Control Loop, the **Adaptation State Vector** (one typed object replacing `C-32`'s four unstructured outputs, persisted in an existing ADR 10 store — no new store), the Adaptive Posture Engine, the **Pressure Model**, the **Adaptation Arbiter**, the **Stability Governor** (dwell, rate limit and hysteresis — authored as Band-2 subtractive rules in the existing policy pack, never as a second evaluator), scaffolding, hints, pace, difficulty, cognitive load, misconception response, the escalation order, and campaign adaptation. **`C-32` keeps the control loop, the target band, the withholding policy and the absolute affect veto; Phase 3 supplies the instrument and takes the adjustment decision (AH-1).** Publishes `AdaptationConstraint` to Band 2 (subtractive) and `AdaptationParameters` to Band 5 (personalization) under EOS v2's seven-band scheme. Defines **no second difference rule** — Phase 1 TQ-4 owns what a different attempt is; Phase 3 owns how much support it carries — and **no adaptation path bypasses a Phase 1 budget** (BB-1…BB-4, AH-12). First phase document to file itself against the Authority Index at row 6 (Advisory). **Architecture only — authorizes no implementation; G1/G2 remain in force.** |
| **Phase 4** | **Educational Assets Architecture** (`PHASE_04_EDUCATIONAL_ASSETS_ARCHITECTURE.md`) | **CANONICAL** (v4.0.2, 2026-07-31) — repository-owner authorization; no review artifact committed (its §9.1 is the approval record) | The educational-asset territory, reconciled rather than designed. Its finding determines its size: **all sixteen briefed items already have owners** — CEKR (Frozen, row 3) owns identity, versioning, provenance, relationships, multilingual variants, discoverability and validation; **ADR 14** the runtime catalogue and lifecycle; **ADR 13** quality evidence and the curator loop; the **AUTHORING_SDK** authoring; and **EBC §8 build-time coverage sufficiency**, under an explicit delegation from **CEKR §11** (*"CEKR validation = 'is this element well-formed and honestly claimed'; EBC validation = 'is this subject shippable'"*). The seventeenth item, a prioritized authoring work queue, **already exists and runs** — `src/lib/teaching/assets/contentQualityDashboard.ts`, documented in no ADR, this Bible or `ENGINE_REFERENCE.md` (EF-5). Phase 4 therefore designs **no asset system** and specifies two seams: **EA-1** demand routing (Phase 1's `ASSET_ABSENT`, Phase 2's visual coverage defect and Phase 3's authoring flag reach the live queue, which derives only from serving telemetry and structurally cannot see an asset never requested because it never existed) and **EA-2** the **sufficiency vector** — coverage as a per-consumer vector rather than a scalar, `coveragePercent` left untouched and reinterpreted. Within EA-2, `teaching` is derived from EBC's E0501/E0502/E0503 and MUST NOT be redefined; `adaptive` is bounded to the runtime depth-versus-need reading because **EBC already owns ladder validity and completeness** (E0506, E0507, §7.6's E0401/E0402, §6.4's E0332); **`visual` is its only wholly new component**; and where EA-2 and an E-code disagree, **the E-code wins**. Records four contradictions it did not fix (CT-1…CT-4) and files them as EH-4. **Architecture only — authorizes no implementation; G1/G2 remain in force and EH-1…EH-5 remain proposals.** |
| ADR 15 | Rendered Reality Model (RRM) | **Proposal** | The LLM has zero cross-turn awareness of what visuals are rendered on the learner's screen — tag stripping destroys provenance, contextSnapshot persists no visual state, `alreadyShown` is a phase-counter heuristic not rendered history. Proposes a server-authoritative, per-turn append-only log of rendered visual state (identity + semantic description + source pipeline + matched concept + turn position), persisted in contextSnapshot, injected into the system prompt as a RENDERED REALITY block. Five invariants (perception-before-reference, server authority, semantic sufficiency, session scope, additive-only). Four-phase incremental migration (persist-only → typed RRM → semantic RRM → learner observation cross-check). Closes the perception loop orthogonal to Signal Verification's evidence loop. |
