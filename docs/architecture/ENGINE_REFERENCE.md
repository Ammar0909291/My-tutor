...
| 16 | Teaching Output Bias | `teachingOutputBias.ts` | **STUB — corrected 2026-07-31 (AH-9).** The file is 13 lines and **every exported function returns a constant**: `deriveOutputBias`/`getOutputBias` always `{ kind: 'BALANCED' }`, `deriveHintBias`/`getHintBias` always `null`, `buildOutputBiasBlock`/`applyHintBias`/`applyOutputBias` always `''`, `isOptionalInlinePractice`/`isOptionalVisual`/`isOptionalVisualTag` always `false`, `isRequiredSceneSpec` always `true`. It computes nothing and gates nothing. The prior description (an engine that "computes output bias / hint bias … gates how much scaffolding the response should include") described an intent, not the code. Recorded as Phase 3 finding **AF-1**/**AC-3**; the `[HINT]` path in `school/tutoring/hintTag.ts` consumes this stub's constant output. Documentation correction only — the file is unchanged. |
