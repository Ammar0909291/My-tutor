...
- **Groq restored (17:10 UTC):** owner updated `GROQ_API_KEY` and redeployed (`dpl_F6ot…`); a 3-turn probe was served 3/3 by Groq. Runs are back on the default `groq,memory`.
...
| S12b | Free-body diagram + normal force, run 2 (prod `d100b41`, Batch 17 + CLOSING fix live) | **8/10 mastered** (was 5/10), **0 critical**, 4 major. Both misses (beginner/normal-force at DEMONSTRATE, confused/free-body at CHECK 1/0) closed at turn 12 on a single wrong answer — the budget extension required 0 consecutive failures. **Changed**: extension now refused only at >=2 consecutive misses (turns only, never mastery) — `conceptBudget.ts`, owner may veto | needs runs 3 |
| S12c | Free-body diagram + normal force, run 3 (prod `dfb870e`, budget-extension rule live) | **10/10 mastered, 0 critical**, 4 major. confused/free-body mastered at turn 18 via the extension. Trend 5/10 → 8/10 → 10/10. Strict readiness needs 3 consecutive clean runs: this is run 1 of 3 | 2 more clean runs |
| S13 | Tension + friction, run 2 (prod `f994ddc`, Groq, Batches 18/19 live) | **10/10 mastered, 0 critical** (run 1: 6/10). Found a real defect the checker mislabelled: confused/tension t12 — the authored key "49 N" was graded correct (`[mcq-grade] correct: true`) but the reply opened "That's not quite right…"; the C5 enforcer passed it because "not quite right" contains "quite right". Fixed in `answerConfirmation.ts` (DENIES_CORRECT; opening denial stripped, confirmation added) + runner `false-correction` regex. offtrack/friction 'invented question' = runner key list loaded before Batch 20 (the probe is authored, ACTIVE, key 0.5) | run 1 of 3 clean |
...
| F15 | 2026-09-25 16:21:32 | 10,852,389 | 190,474,368 | end of free-body + normal-force run 2 (+ Batch 19 SQL, seeder-fix deploy): +188,580 rows, about 57 MB. **Today ≈ 379 MB** (cap 1 GB) |
| F16 | 2026-09-25 17:13:36 | 10,883,310 | 190,717,249 | end of free-body + normal-force run 3 (+ seeder cold starts): +242,881 rows since F15, about 73 MB. **Today ≈ 452 MB** (cap 1 GB) |
| F17 | 2026-09-25 17:48:37 | 10,931,006 | 191,213,730 | tension + friction run 2 + 5 deploys + seeder cold starts: +496,481 rows, about 149 MB. **Today ≈ 601 MB** (cap 1 GB). The seeder's full prefetch on every cold start is the suspected extra cost until seeding completes (~20 cold starts left); per-query baseline saved for attribution |
...
**Update 2026-09-25 ~16:00 UTC — BLOCKER RESOLVED (owner "Both"):** root cause of the stall found from the phase-timing logs: the prefetch read only BOOTSTRAP_SEED_SUBJECTS-tagged rows, so the 992 SQL-applied biology rows looked missing; the planner re-inserted the same first 150 biology explanations every cold start and the seed-slug unique index skipped them all (`created=0 spent=150/150` forever). Fixed in `2be8d81` (presence = the unique index's scope), deployed in `1cc044d`. Expect ~150 new rows per cold start (mathematics first) until complete. Batch 19 (22 probes) applied by SQL and verified 22/22 ACTIVE, 4 choices each.
