...
export async function POST(req: NextRequest) {
  ...
  const chapters = getSchoolChapters(educationBoard, ps.subjectSlug, grade)
  ...
  // CRITICAL-4 (Sprint D): StudentProgress and TopicProgress used to be
  // written via separate, non-transactional statements. An interrupted
  // request between them could leave a chapter marked "complete" in one
  // model while the other still shows IN_PROGRESS/un-mastered, with the
  // completedAt claim above already blocking any retry. Wrap both writes
  // in a single transaction so they always succeed or fail together.
  await withRetry(() => prisma.$transaction(async (tx) => {
    // If passed: mark chapter complete in StudentProgress
    if (result.passed && ps.chapterId) {
      const code = schoolSubjectCode(educationBoard, ps.subjectSlug, grade)
    ...
    if (chapter.kgNodeIds.length > 0) {
      const existing = await tx.topicProgress.findMany({
  ...
  Promise.resolve().then(async () => {
    ...
    await checkAndUnlockAchievements(session.user.id, educationBoard, grade)
  }).catch(() => {})
