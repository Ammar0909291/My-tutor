...
import { generateChapterPractice } from '@/lib/school/practice/generateChapterPractice'
...
export async function POST(req: NextRequest) {
  ...
  try {
    const questions = await generateChapterPractice(board, subjectSlug, subjectName, grade, chapter)
    ...
    console.error('[practice/generate]', error)
    captureError(error, { route: 'api/school/practice/generate', tags: { subjectSlug } })
    return NextResponse.json({ error: 'Could not generate practice questions. Please try again.' }, { status: 500 })
  }
}
