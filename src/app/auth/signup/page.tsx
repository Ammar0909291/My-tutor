...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(false)
    if (result?.error) {
      setError('Ошибка входа после регистрации. Попробуй войти вручную.')
    } else {
    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="relative w-full max-w-md animate-slide-up">
        <div className="text-center mb-8">
          ...
          <h1 className="mt-7 text-2xl font-bold text-white">Создай аккаунт</h1>
          <p className="mt-2 text-slate-400 text-sm">Первый урок бесплатно, без карты</p>
        </div>
        ...
        <p className="text-center mt-6 text-sm text-slate-400">
          Уже есть аккаунт?{' '}
          <Link href="/auth/login" className="text-accent-400 font-medium hover:text-accent-300 transition-colors">
            Войти
          </Link>
        </p>
      </div>
