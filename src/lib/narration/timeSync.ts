 ...
 * words' character lengths (plus each word's own trailing whitespace gap —
 * see `buildWordTimeWindows` below for why that gap matters). This is a
 * SECOND layer of estimate on top of the segment-level one (itself already
 * an estimate): no provider this
 ...
 * proportionally across its RENDERED words — word-level highlighting for the
 * server-audio path, built entirely from information already on hand (no
 * second rendered/spoken word-count reconciliation is needed here, unlike
 * the browser-speech path, because this subdivides the segment's window
 * directly by its RENDERED words rather than by anything derived from the
 * spoken audio). A segment with zero rendered words (should not occur)
 * contributes no word windows.
 *
 * WEIGHTING INCLUDES THE WORD'S OWN TRAILING GAP, NOT JUST ITS CHARACTERS.
 * Real speech spends real time on the brief gap between one word and the
 * next, not only on the word's own letters — and that gap is a much LARGER
 * share of a short word's true speaking time than of a long one's (a
 * one-syllable "the" has proportionally more of its slot taken up by
 * onset/gap overhead than a five-syllable word does). Weighting purely by a
 * word's own character count (the prior version of this function) ignores
 * that gap-time entirely, so it systematically UNDER-allocates short,
 * common words — the words ordinary teaching prose is dense with — and
 * OVER-allocates long ones. Across a sentence this compounds: the estimated
 * "current word" advances past each short word faster than it is really
 * spoken, so the visible highlight measurably runs ahead of the voice by
 * the time a segment is halfway through (reproduced and measured in
 * `narrationTimeSync.test.ts`'s "highlight must never run ahead" block).
 *
 * The correction uses only whitespace ALREADY present in the segment's own
 * tokenization (`renderedWords`'s 'space' tokens) — never an invented
 * constant or a global multiplier — so a word's weight is its own character
 * length plus the length of any whitespace run immediately following it (0
 * for a segment's final word, which has no following gap to attribute).
 * This can only ever REDUCE how far ahead the estimate runs relative to the
 * pure-character-length version — it is monotonically no worse, and directly
 * closes the specific bias measured above.
 */
export function buildWordTimeWindows(segments: NarrationSegment[], totalDuration: number): WordTimeWindow[] {
  ...
  for (let i = 0; i < segmentWindows.length; i++) {
    ...
    if (!segment) continue
    const tokens = segment.renderedWords
    const wordIndices: number[] = []
    const lengths: number[] = []
    for (let t = 0; t < tokens.length; t++) {
      const token = tokens[t]
      if (token.kind !== 'word') continue
      const gap = tokens[t + 1]?.kind === 'space' ? tokens[t + 1].text.length : 0
      wordIndices.push(token.wordIndex as number)
      lengths.push(Math.max(token.text.length + gap, 1))
    }
    ...
    for (let w = 0; w < wordIndices.length; w++) {
      ...
      result.push({ segmentIndex: segment.index, wordIndex: wordIndices[w], startTime, endTime })
    }
  }
  return result
}
