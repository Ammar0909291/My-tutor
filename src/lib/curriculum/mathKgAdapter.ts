/**
 * Step A: 908-node KG → application type adapter
 *
 * Bridges docs/mathematics/kg/graph.json (908-node canonical KG) to the two
 * type shapes the app needs:
 ...
 *   id prefix       → domain  (e.g. "math.found.xxx" → "math.found")
...
function getRawConcepts(): RawConcept[] {
  ...
  const filePath = path.join(process.cwd(), 'docs/mathematics/kg/graph.json')
...
/**
 * Returns all 908 KG concepts as KnowledgeNode[].
 * Used by resolveNodes('mathematics') in knowledgeGraph.ts.
 */
export function getMathKgNodes(): KnowledgeNode[] {
 ...
 * Engine.  Returns undefined if the id is not in the 908-node KG.
 */
export function getMathConceptNode(id: string): ConceptNode | undefined {
