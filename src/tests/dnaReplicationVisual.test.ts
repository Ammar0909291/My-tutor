...
describe('5-6. nothing else moved', () => {
  ...
  it('the other molecular-biology DNA concepts still without a fix resolve exactly as before (no figure)', () => {
    // bio.mol.nucleic-acid-structure, bio.mol.transcription, and (as of
    // batch 7) bio.mol.dna-damage-repair were given their own Tier 0
    // scenes by the concurrent Biology visual coverage campaign
    // (2026-09-25) after the same live sweep that motivated this file
    // found them stuck too — see bioVisualCoverageCampaign.test.ts. That
    // is a real, intended change to their resolution, not a regression of
    // this test's own DNA-replication fix.
