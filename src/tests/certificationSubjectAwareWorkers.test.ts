 ...
 * REQUEST must never widen what may RUN. Every refusal — missing credential
 * pair, protected account, per-slot designation, two workers on one account —
 * applies to the requested slots exactly as before, and the numeric path that
 * every other caller uses is unchanged.
 *
...
import { resolveWorkers, isProtectedAccount } from '../../scripts/certification/measurementIdentity'
...
describe('narrowing the request never weakens a refusal', () => {
  it('a protected account in a REQUESTED slot is still refused', () => {
    const bad = { ...envAll(4), CERT_WORKER_2_EMAIL: 'suaibamr@gmail.com' }
    const r = resolveWorkers(bad, [2, 4])
    expect(r).toMatchObject({ ok: false })
    if (!r.ok) expect(r.error).toMatch(/protected account/)
    // And the general predicate is untouched, exactly as the numeric path.
    expect(isProtectedAccount('suaibamr@gmail.com')).toBe(true)
  })

  it('the per-slot designation still works, and still only for its own slot', () => {
    const designated = {
      ...envAll(4),
      CERT_WORKER_2_EMAIL: 'suaibamr@gmail.com',
      CERT_WORKER_2_DESIGNATED_TEST_ACCOUNT: 'true',
    }
    const ok = resolveWorkers(designated, [2, 4])
    expect(ok.ok).toBe(true)

    // A designation on a slot that is not the offending one must not help it,
    // whether or not that slot is even part of this request.
    const wrongSlot = {
      ...envAll(4),
      CERT_WORKER_2_EMAIL: 'suaibamr@gmail.com',
      CERT_WORKER_3_DESIGNATED_TEST_ACCOUNT: 'true',
    }
    const refused = resolveWorkers(wrongSlot, [2, 4])
    expect(refused).toMatchObject({ ok: false })
    if (!refused.ok) expect(refused.error).toMatch(/protected account/)
  })
