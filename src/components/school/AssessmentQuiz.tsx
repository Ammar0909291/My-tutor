...
export function AssessmentQuiz({ subjectSlug, chapterId, chapterTitle, recommendPractice, navigatorAction }: AssessmentQuizProps) {
  ...
  async function submitAnswers() {
    ...
    try {
      ...
      void persistVisualMasterySummary({ subjectSlug, topicSlug: chapterId, source: 'assessment', sessionId, summary })
    } catch {
      ...
      <Shell backLabel="Back to chapter">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {/* Sprint Q: read-only visual learning insight — built entirely from
              already-persisted visual mastery evidence, never affects score/grading/pass-fail. */}
