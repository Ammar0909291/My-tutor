...
| Concepts with an Educational Brain entry | **512** mathematics + 238 physics + 216 english + 186 chemistry + 0 biology + 0 computer_science = **1,152** |
| Remaining | **623** |
| Completion percentage | **64.90%** |

*(Recomputed programmatically 2026-09-13, after Batch 74 (4 concepts:
...
|---|---|---|---|---|---|
| mathematics | 908 | 512 | 56.39% | `math.found.mathematical-thinking` | **Yes** |
...
| `math.calc` | 75/76 | **REOPENED (2026-09-13) — Batch 74's `math.linalg.determinant` unblocked
...
| `math.linalg` | 12/61 | **Batch 74 (4, `determinant`/`norm`/`orthogonality`/`symmetric-matrix`,
...
**Nine domains certified** (math.found, math.geom, math.arith, math.nt, math.alg, math.func,
math.trig, math.seq, and — as of Batch 73 — **math.disc, the ninth**); as of Batch 70, math.disc,
...
**REOPENS `math.calc`**: `math.calc.change-of-variables` is now ready, the domain's FINAL
concept. `math.graph` remains PARKED (needs `math.linalg.eigenvalues`/
`math.prob.probability-axioms`, neither yet authored). `math.linalg`'s own fresh frontier now has
9 candidates ready (`unit-vector`, `cross-product`, `linear-system`, `matrix-inverse`,
`cofactor-expansion`, `det-properties`, `eigenvalues`, `distance`, `angle-vectors`) — authoring
`eigenvalues` would also reopen `math.graph`. Mathematics stands at 512 of 908. The nine certified
