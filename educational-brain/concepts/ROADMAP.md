...
| Concepts with an Educational Brain entry | **500** mathematics + 238 physics + 216 english + 186 chemistry + 0 biology + 0 computer_science = **1,140** |
| Remaining | **635** |
| Completion percentage | **64.23%** |

*(Recomputed programmatically 2026-09-13, after Batch 71 (1 concept: `math.linalg.vector`, the
...
`math.linalg.vector`; the fresh frontier now has 4 candidates ready
(`vector-addition`/`scalar-multiplication`/`dot-product`/`matrix`), and authoring `matrix` will
be the direct step that reopens `math.disc.graph-representation`, closing that domain entirely.
Mathematics stands at 500 of 908. The eight certified
