...
| Concepts with an Educational Brain entry | **680** |
| Remaining | **1,095** |
| Completion percentage | **38.31%** |

*(Recomputed from §2 Subject progress table: 224 mathematics + 238 physics
+ 32 english + 186 chemistry + 0 biology + 0 computer_science = 680.
English updated after Batch 8 of the English Educational Brain authoring
program (4 concepts, topological level 9); the other subjects' figures
...
| english | 216 | 32 | 14.81% | `eng.phonics.phonemic-awareness`, `eng.phonics.print-concepts` | Yes (both) |
...
**English Batch 8 (this batch)**: continuing directly after Batch 7.
Recomputed the topological queue fresh — confirmed exactly 4 concepts at
level 9: `eng.listening.active-listening`, `eng.phonetics.rhythm-and-
timing`, `eng.speaking.oral-fluency`, `eng.vocab.word-recognition`. All 4
had existing Blueprints, reused by reference. This batch introduced the
first `eng.listening.*` and `eng.speaking.*` domain entries in this
program (both with distinct audio/non-auditory evidence profiles from
the phonics/phonetics domains authored so far), confirmed the missing-
physics-cross-link pattern at a twelfth site (`rhythm-and-timing`,
itself flagged as having a population-dependent birth type), and
identified an overcorrection-pair parallel between `oral-fluency`'s
misconceptions and `eng.phonics.decoding-fluency`'s own — full detail in
`COVERAGE.md`'s Delivery history. English EB reached 32/216 (14.81%).
True total: **680** EB entries (238 physics + 224 mathematics + 32
english + 186 chemistry), out of 1,775 total KG concepts — 1,095
remaining, 38.31%. Next candidates (topological level 10, to be
confirmed fresh): `eng.grammar.word-classes-overview`, `eng.listening.
distinguishing-sounds-in-speech`, `eng.listening.listening-for-gist`,
`eng.phonetics.accents-and-dialects`, `eng.speaking.pronunciation-in-
conversation`, `eng.vocab.context-clues`, `eng.vocab.synonyms-antonyms`,
`eng.vocab.word-families`.

**English Batch 7 (prior batch)**: continuing directly after Batch 6.
Recomputed the topological queue fresh — confirmed exactly 4 concepts at
level 8: `eng.phonetics.connected-speech`, `eng.phonetics.prosody`,
`eng.phonics.decoding-fluency`, `eng.writing.spelling-strategies`. All 4
had existing Blueprints, reused by reference. Identified `eng.phonetics.
prosody` as this program's second synthesis concept, with its first
misconception caused specifically by this program's own prior teaching
sequence — flagged as a general risk for future integrating concepts.
Confirmed the missing-physics-cross-link pattern at two further sites
(tenth, eleventh occurrences) and a second, larger-scale instance of the
presentation-format-contamination sub-pattern — full detail in
`COVERAGE.md`'s Delivery history. English EB reached 28/216 (12.96%).
True total: **676** EB entries (238 physics + 224 mathematics + 28
english + 186 chemistry), out of 1,775 total KG concepts — 1,099
remaining, 38.08%. Next candidates (topological level 9, to be confirmed
fresh): `eng.listening.active-listening`, `eng.phonetics.rhythm-and-
timing`, `eng.speaking.oral-fluency`, `eng.vocab.word-recognition`.

**English Batch 6 (prior batch)**: continuing directly after Batch 5.
Recomputed the topological queue fresh — confirmed exactly 2 concepts at
level 7: `eng.phonetics.intonation-patterns`, `eng.phonics.syllable-
types`. Both had existing Blueprints, reused by reference. Confirmed the
missing-physics-cross-link pattern at a ninth site (inherited, not
independently discovered) and identified the largest-scale single-rule-
overgeneralisation instance yet in this program (`syllable-types`' six
categories) — full detail in `COVERAGE.md`'s Delivery history. English EB
reached 24/216 (11.11%). True total: **672** EB entries (238 physics +
224 mathematics + 24 english + 186 chemistry), out of 1,775 total KG
concepts — 1,103 remaining, 37.86%. Next candidates (topological level 8,
to be confirmed fresh): `eng.phonetics.connected-speech`, `eng.phonetics.
prosody`, `eng.phonics.decoding-fluency`, `eng.writing.spelling-
strategies`.

**English Batch 5 (prior batch)**: continuing directly after Batch 4.
Recomputed the topological queue fresh from the live KG — confirmed
exactly 3 concepts at level 6: `eng.phonetics.phonetic-transcription`,
`eng.phonetics.sentence-stress`, `eng.phonics.digraphs`. All 3 had
existing Blueprints, reused by reference. Confirmed the missing-physics-
cross-link pattern is inherited/systemic across the whole
`eng.phonetics.*` domain rather than concept-by-concept. Identified a
genuinely new overgeneralisation sub-category (importing "one correct
answer" into a domain that is honestly multi-valued) and a Blueprint
coverage gap (MP-5 at `eng.phonics.digraphs` doesn't fully verify its
second misconception's repair) — full detail in `COVERAGE.md`'s Delivery
history. English EB reached 22/216 (10.19%). True total: **670** EB
entries (238 physics + 224 mathematics + 22 english + 186 chemistry), out
of 1,775 total KG concepts — 1,105 remaining, 37.75%. Next candidates
(topological level 7, to be confirmed fresh): `eng.phonetics.intonation-
patterns`, `eng.phonics.syllable-types`.

**English Batch 4 (prior batch)**: continuing directly after Batch 3.
...
recurring cross-batch misconception-birth-type patterns (notation-induced
reasoning-from-appearance, and terminology-contamination now recurring at
the category-label level) and one genuine, deliberate absence of the
physics-cross-link pattern at `ipa-basics` — full detail in `COVERAGE.md`'s
Delivery history. English EB reached 15/216 (6.94%). True total: **663**
EB entries (238 physics + 224 mathematics + 15 english + 186 chemistry),
out of 1,775 total KG concepts — 1,112 remaining, 37.35%. Next candidates
...
**English Batch 2 (prior batch)**: continuing directly after Batch 1.
...
topological authoring queue from `docs/english/kg/graph.json` (213 missing
concepts, levels 1–39). Authored the full level-1/level-2 frontier (6
concepts): `eng.phonetics.speech-sounds-overview`, `eng.phonics.alphabet-
recognition`, `eng.phonics.rhyming` (level 1); `eng.phonetics.articulation-
