...
| Concepts with an Educational Brain entry | **516** mathematics + 238 physics + 216 english + 186 chemistry + 0 biology + 0 computer_science = **1,156** |
| Remaining | **619** |
| Completion percentage | **65.13%** |

*(Recomputed programmatically 2026-09-13, after Batch 75 (4 concepts:
...
|---|---|---|---|---|---|
| mathematics | 908 | 516 | 56.83% | `math.found.mathematical-thinking` | **Yes** |
...
| `math.linalg` | 15/61 | **Batch 75 (3, `eigenvalues`/`unit-vector`/`cross-product`, selected
...
| `math.graph` | 14/16 | **REOPENED (2026-09-13) — Batch 75's `math.linalg.eigenvalues`
...
`eigenvalues` **REOPENED `math.graph`**: `math.graph.algebraic-graph-theory` is now ready,
confirmed via a fresh frontier check, leaving only `random-graph` (needs
`math.prob.probability-axioms`, unauthored) before that domain's own certification. `math.linalg`'s
own fresh frontier now has 7 candidates ready (`linear-system`, `matrix-inverse`,
`cofactor-expansion`, `det-properties`, `characteristic-polynomial`, `distance`,
`angle-vectors`). Mathematics stands at 516 of 908. The ten certified
