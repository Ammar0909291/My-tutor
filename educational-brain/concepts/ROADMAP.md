...
`eng.grammar.*` entry), `eng.listening.distinguishing-sounds-in-speech`
(first two-domain-convergent prerequisite pair, confirmed terminal),
`eng.listening.listening-for-gist`, `eng.phonetics.accents-and-dialects`
(this program's most affect-sensitive concept to date, crossing into
`eng.linguistics.*` via its unlock). All 4 had existing Blueprints,
