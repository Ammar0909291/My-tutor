...
| english | 216 | 120 | 106/216 authored across ~46+ batches tracked via commit messages plus 7 concepts reconciled in from a parallel feature branch 2026-08-11 plus Batch 10 (4), Batch 11 (5), Batch 12 part 1 (3: `eng.listening.following-instructions`, `eng.listening.note-taking-while-listening`, `eng.linguistics.phonology-intro`), Batch 12 part 2 (2, autonomous /loop: `eng.linguistics.morphology-intro`, `eng.reading.close-reading`) — this row was stale at 6 since 2026-08-04; see `ROADMAP.md` §6 and `AUTHORING_QUEUE.md`'s reconciliation notes for the full audit; per-batch narrative for the pre-reconciliation history lives in git commit messages, not here |
