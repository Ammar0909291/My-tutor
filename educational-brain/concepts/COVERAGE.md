...
| english | 216 | 106 | 106/216 authored across ~46+ batches tracked via commit messages (phonics/phonetics/vocab/grammar domains complete, `eng.reading.*` in progress through `text-structure`) plus 7 concepts reconciled in from a parallel feature branch 2026-08-11 (`eng.listening.active-listening`, `eng.listening.distinguishing-sounds-in-speech`, `eng.listening.listening-for-gist`, `eng.speaking.oral-fluency`, `eng.speaking.pronunciation-in-conversation`, `eng.writing.handwriting-and-formation`, `eng.writing.spelling-strategies`) — this row was stale at 6 since 2026-08-04; see `ROADMAP.md` §6 and `AUTHORING_QUEUE.md`'s reconciliation notes for the full audit; per-batch narrative for the pre-reconciliation history lives in git commit messages, not here |
