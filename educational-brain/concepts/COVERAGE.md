...
| english | 216 | 40 | Pre-existing 3: `eng.phonics.letter-sound-correspondence`, `eng.phonics.phonemic-awareness`, `eng.phonics.print-concepts`. Batch 1 (2026-08-03): `eng.phonetics.speech-sounds-overview`, `eng.phonics.alphabet-recognition`, `eng.phonics.rhyming`, `eng.phonetics.articulation-organs`, `eng.phonics.blending-segmenting`, `eng.writing.handwriting-and-formation` (levels 1–2). Batch 2 (2026-08-03): `eng.phonetics.consonant-sounds`, `eng.phonetics.vowel-sounds` (level 3). Batch 3 (2026-08-03): `eng.phonetics.ipa-basics`, `eng.phonics.consonants`, `eng.phonics.short-vowels`, `eng.phonics.sight-words` (level 4). Batch 4 (2026-08-03): `eng.phonetics.minimal-pairs`, `eng.phonetics.syllable-stress`, `eng.phonics.consonant-blends`, `eng.phonics.long-vowels-silent-e` (level 5). Batch 5 (2026-08-03): `eng.phonetics.phonetic-transcription`, `eng.phonetics.sentence-stress`, `eng.phonics.digraphs` (level 6). Batch 6 (2026-08-03): `eng.phonetics.intonation-patterns`, `eng.phonics.syllable-types` (level 7). Batch 7 (2026-08-03): `eng.phonetics.connected-speech`, `eng.phonetics.prosody`, `eng.phonics.decoding-fluency`, `eng.writing.spelling-strategies` (level 8). Batch 8 (2026-08-03): `eng.listening.active-listening`, `eng.phonetics.rhythm-and-timing`, `eng.speaking.oral-fluency`, `eng.vocab.word-recognition` (level 9). Batch 9 part 1 (2026-08-03): `eng.grammar.word-classes-overview`, `eng.listening.distinguishing-sounds-in-speech`, `eng.listening.listening-for-gist`, `eng.phonetics.accents-and-dialects` (level 10). Batch 9 part 2 (2026-08-03): `eng.vocab.context-clues`, `eng.vocab.synonyms-antonyms`, `eng.vocab.word-families`, `eng.speaking.pronunciation-in-conversation` (level 10). 176 concepts remain; see Delivery history below. |
...
### English — Batch 1: topological levels 1–2 (2026-08-03, English Educational Brain authoring program)

Standing program restarted per explicit task instruction: bring English's
Educational Brain corpus from 3/216 to 216/216, working strictly on
`educational-brain/concepts/english/*.md`, no runtime/KG/Blueprint/Teaching
Asset changes. Audited current state first: English KG (216/216), Blueprints
(216/216, `docs/curriculum/blueprints/eng.*.md`), and Teaching Assets
(216/216, pipeline-owned) are all complete; only the Educational Brain layer
is incomplete, at 3/216 (`eng.phonics.letter-sound-correspondence`,
`eng.phonics.phonemic-awareness`, `eng.phonics.print-concepts`). Computed the
exact topological authoring queue directly from `docs/english/kg/graph.json`
(level = distance from a root with no unmet prerequisites among remaining
concepts) — 213 concepts missing, levels 1 through 39.

Authored the 6 concepts at topological levels 1–2 (the full unlocked
frontier immediately above the 3 pre-existing entries): `eng.phonetics.
speech-sounds-overview`, `eng.phonics.alphabet-recognition`, `eng.phonics.
rhyming` (level 1); `eng.phonetics.articulation-organs`, `eng.phonics.
blending-segmenting`, `eng.writing.handwriting-and-formation` (level 2). All
6 had existing, substantial Blueprints (`docs/curriculum/blueprints/eng.*.md`,
Components 0–9) — every entry reuses the Blueprint's Misconception Register,
Concrete Anchor, Conceptual Development Sequence, Worked Examples, Mastery
Probe Set, and Session Architecture **by reference**, adding only what the
Blueprint does not own: birth-type classification per misconception, Core
Understanding, the four-stage Mental Models progression, Why Students Fail,
ranked Analogies with explicit Anti-Analogies, response-*pattern* reads for
the assessment signals (vs. the Blueprint's item bank), Tutor Recovery
Strategy, Memory Hooks, Transfer/Cross-Subject Connections, and Curriculum
Feedback. 3 new misconceptions not present in any Blueprint were authored
directly via the birth-taxonomy diagnostic procedure (`../misconceptions/
01-birth-taxonomy.md`): MC-FINAL-CONSONANT-IS-THE-RIME (rhyming),
MC-DIAGRAM-IS-THE-TERRITORY (articulation organs) — both recorded as
Curriculum Feedback recommending Blueprint addition, not added to the
Blueprints directly (out of this program's scope).

Genuine cross-subject findings recorded as Curriculum Feedback, not fixed
(no KG file touched): `eng.phonetics.speech-sounds-overview` and
`eng.phonetics.articulation-organs` both have real, mechanism-level missing
`cross_links` to physics (voicing = vocal-fold vibration frequency; vowel
quality = source-filter resonance) that the KG currently encodes as `[]`;
`eng.phonetics.articulation-organs` additionally has a missing biology link
(anatomy of the vocal tract). `eng.phonics.alphabet-recognition` has a
missing mathematics link (digit/symbol recognition shares the arbitrary-
shape-to-name mapping and the mirror-hazard problem, e.g. 6/9). Bloom-level
mismatches (`remember` under-describing what are actually `understand`/
`apply`-level discrimination or categorisation tasks) were flagged at 3 of
the 6 entries (`alphabet-recognition`, `rhyming`, `handwriting-and-
formation`) as a recurring pattern worth the Pipeline's attention, not a
one-off.

English 3/216 → **9/216**. 207 concepts remain. Next candidates (topological
level 3, computed fresh): `eng.phonetics.consonant-sounds`,
`eng.phonetics.vowel-sounds` — both direct children of `eng.phonetics.
articulation-organs`, both with existing Blueprints. `EDUCATIONAL_BRAIN_
INDEX.md`, `AUTHORING_QUEUE.md`, `ROADMAP.md`, and `QUALITY.md` to be
regenerated in the same commit as this entry, per `PRODUCTION_PIPELINE.md`'s
standard workflow. Full validation before commit: KG validator PASS (all 6
...
### English — Batch 3: topological level 4 (2026-08-03, English Educational Brain authoring program)

Continuing directly after Batch 2. Authored the 4 concepts at topological
level 4: `eng.phonetics.ipa-basics` (unlocked by both level-3 phonetics
concepts), `eng.phonics.consonants`, `eng.phonics.short-vowels`,
`eng.phonics.sight-words` (all three unlocked by the pre-existing
`eng.phonics.letter-sound-correspondence`). All 4 had existing, substantial
Blueprints, reused by reference for the same components as prior batches.

Notable findings this batch: (1) `eng.phonetics.ipa-basics`' two
misconceptions are both Type 4 (notation-induced), extending a pattern
first identified at `eng.phonics.rhyming` and `eng.phonetics.articulation-
organs` — reasoning from a notation's surface appearance rather than what
it represents — now recurring at a fourth site; (2) `eng.phonics.sight-
words`' first misconception extends the *related but distinct* terminology-
contamination pattern (Type 3) to the category-*label* level itself ("sight
word" sounding like "unreadable by sound"), rather than a single word or
symbol, joining `eng.phonetics.speech-sounds-overview`'s "sound"
contamination as a second data point; both patterns are now recorded as a
single consolidated recommendation (in `ipa-basics`' Curriculum Feedback)
that the Pipeline consider a notation/terminology-contamination lens when
reviewing concepts that introduce any new written or category-label system.
(3) `eng.phonetics.ipa-basics` was confirmed to have NO missing physics
cross-link, in explicit contrast to all four preceding phonetics concepts —
recorded as a genuine, deliberate absence (notation is not itself a
physical mechanism) rather than a fifth instance of the pattern.
(4) `eng.phonics.consonants` and `eng.phonics.short-vowels` both required
distinguishing two structurally different misconception-repair strategies
sharing the same Type 1 (overgeneralization) birth type — one resolved by
extracting a general *rule* (hard/soft c/g; check-for-silent-e), the other
by learning a small, bounded, unruled *list* (silent letters) — a
distinction worth flagging for the Pipeline's misconception-authoring
guidance generally, not specific to either concept.

English 11/216 → **15/216**. 201 concepts remain. Next candidates
(topological level 5): `eng.phonetics.minimal-pairs`, `eng.phonetics.
syllable-stress`, `eng.phonics.consonant-blends`, `eng.phonics.long-vowels-
silent-e` — all now unlocked, all with existing Blueprints. All five
tracking files updated in the same commit. Full validation: KG validator
...
### English — Batch 5: topological level 6 (2026-08-03, English Educational Brain authoring program)

Continuing directly after Batch 4. Recomputed the topological queue fresh
from the live KG rather than assuming the prior batch's forward-looking
guess — confirmed exactly 3 concepts at level 6: `eng.phonetics.phonetic-
transcription` (unlocked by both `eng.phonetics.ipa-basics` and
`eng.phonetics.minimal-pairs`), `eng.phonetics.sentence-stress` (unlocked
by `eng.phonetics.syllable-stress`), `eng.phonics.digraphs` (unlocked by
`eng.phonics.consonant-blends` and `eng.phonics.short-vowels`). All 3 had
existing, substantial Blueprints, reused by reference.

Notable findings: (1) `eng.phonetics.phonetic-transcription`'s first
misconception is the seventh identified instance of the notation-induced
pattern, and its second misconception — expecting one single correct
transcription — is a genuinely new category within this program's
overgeneralisation catalogue: an overgeneralisation of "one correct
answer" imported from nearly every other skill in the curriculum, now
meeting a domain (real dialect variation) that is honestly, legitimately
multi-valued; (2) the missing-physics-cross-link pattern was confirmed
inherited rather than independently re-discovered at both
`phonetic-transcription` (synthesising every underlying phonetics
concept's acoustic content) and `eng.phonetics.sentence-stress` (a direct
scaling-up of `eng.phonetics.syllable-stress`'s identical link) — the
Pipeline audit recommendation is now understood to be systemic across the
whole `eng.phonetics.*` domain rather than concept-by-concept;
(3) `eng.phonics.digraphs`' first misconception is notable as the freshest,
most-immediate instance of negative transfer catalogued in this program —
drawing on knowledge secured in the literal preceding lesson
(`eng.phonics.consonant-blends`) rather than knowledge from well earlier
in the curriculum, a specific sub-pattern worth flagging to the Pipeline
for concepts that immediately follow a closely related prior concept;
(4) `eng.phonics.digraphs`' second misconception was clarified as
targeting a genuinely different kind of repair than this program's other
rule-plus-exception-list concepts — a calibrated-confidence stance for a
category that is honestly less reliable, not a completable exception
list — and a genuine Blueprint coverage gap was found (MP-5 doesn't fully
verify this repair) and recorded as Curriculum Feedback, not fixed;
(5) an informal cross-concept dependency was identified between
`eng.phonetics.phonetic-transcription`'s connected-speech task and
`eng.phonetics.sentence-stress`, not encoded in the KG's formal
prerequisite graph — recorded for the Pipeline's awareness, not treated
as a defect since both concepts' own prerequisite chains remain
independently well-formed.

English 19/216 → **22/216**. 194 concepts remain. Next candidates
(topological level 7, to be confirmed fresh next batch): `eng.phonetics.
intonation-patterns`, `eng.phonics.syllable-types`. All five tracking
files updated in the same commit. Full validation: KG validator PASS (all
6 subjects, English 216/216 reachable, unchanged), `npx tsc --noEmit`
clean, existing test suite green, no runtime/KG/Blueprint/Teaching Asset
file touched.

### English — Batch 6: topological level 7 (2026-08-03, English Educational Brain authoring program)

Continuing directly after Batch 5. Recomputed the topological queue fresh
from the live KG — confirmed exactly 2 concepts at level 7:
`eng.phonetics.intonation-patterns` (unlocked by `eng.phonetics.sentence-
stress`) and `eng.phonics.syllable-types` (unlocked by `eng.phonics.
digraphs` and `eng.phonics.long-vowels-silent-e`). Both had existing,
substantial Blueprints, reused by reference.

Notable findings: (1) the missing-physics-cross-link pattern
(fundamental frequency/pitch) was confirmed at `eng.phonetics.intonation-
patterns` as the ninth site, again inherited from its direct prerequisite
rather than independently discovered — the systemic, domain-wide
character of this finding continues to be confirmed rather than
re-discovered batch by batch; (2) `eng.phonics.syllable-types`' second
misconception (defaulting to the closed-syllable rule across all six
types) was identified as the largest-scale instance yet of this
program's recurring single-rule-overgeneralisation pattern — five
alternative categories competing with one over-practised default, versus
one or two alternatives at every prior instance (`eng.phonics.consonants`'
hard/soft c/g, `eng.phonics.digraphs`' blend/digraph carryover) — recorded
as a scaling benchmark for the Pipeline, confirming the same "establish
the default securely, then explicitly widen scope" repair strategy holds
at larger scale; (3) `eng.phonetics.intonation-patterns`' first
misconception (all-questions-rise) was identified as a rule-scope-
narrowing repair structurally parallel to `eng.phonics.consonants`' c/g
rule, and its second misconception (intonation-as-decorative) was
identified as requiring a genuinely new category of meaning-bearing
signal to be added alongside, not in contradiction of, everything
previously taught about word- and grammar-level meaning.

English 22/216 → **24/216**. 192 concepts remain. Next candidates
(topological level 8, to be confirmed fresh next batch): `eng.phonetics.
connected-speech`, `eng.phonetics.prosody`, `eng.phonics.decoding-
fluency`, `eng.writing.spelling-strategies`. All five tracking files
updated in the same commit. Full validation: KG validator PASS (all 6
subjects, English 216/216 reachable, unchanged), `npx tsc --noEmit`
clean, existing test suite green, no runtime/KG/Blueprint/Teaching Asset
file touched.

### English — Batch 7: topological level 8 (2026-08-03, English Educational Brain authoring program)

Continuing directly after Batch 6. Recomputed the topological queue fresh
from the live KG — confirmed exactly 4 concepts at level 8:
`eng.phonetics.connected-speech` (unlocked by `eng.phonetics.intonation-
patterns`), `eng.phonetics.prosody` (unlocked by `eng.phonetics.
intonation-patterns` and `eng.phonetics.phonetic-transcription` — the
second synthesis concept in this program, after transcription itself),
`eng.phonics.decoding-fluency` and `eng.writing.spelling-strategies`
(both unlocked by `eng.phonics.syllable-types`, plus `eng.phonics.sight-
words` and `eng.writing.handwriting-and-formation` respectively). All 4
had existing, substantial Blueprints, reused by reference.

Notable findings: (1) `eng.phonetics.prosody` is this program's second
synthesis concept (after `eng.phonetics.phonetic-transcription`),
integrating stress, rhythm, and intonation — its first misconception was
identified as a novel variant of the instruction-induced pattern, caused
specifically by this program's OWN prior teaching sequence (three
concepts emphasising intonation immediately before this one) rather than
by any external prior instruction, worth flagging to the Pipeline as a
general risk whenever a curriculum deliberately sequences component
skills before an integrating concept; (2) the missing-physics-cross-link
pattern was confirmed at two further sites (`connected-speech` —
coarticulation, tenth occurrence; `prosody` — the full acoustic bundle,
eleventh occurrence, again inherited from synthesised components);
(3) a second, larger-scale instance of the presentation-format-induced
contamination sub-pattern (first found at `connected-speech`'s own
unstructured-example-list case) was identified at `eng.writing.spelling-
strategies`, where the contaminating format is spelling's entire
conventional instructional delivery as a separately-drilled subject,
suggesting this sub-pattern operates at multiple scales; (4) `eng.phonics.
decoding-fluency`'s two misconceptions were identified as a clean
predictable-overcorrection pair (accuracy-is-sufficient, then the
overcorrection to speed-above-all), with the comprehension-check-as-
certification-instrument design flagged as a strong pattern worth
generalising to any future concept where superficial performance risks
being mistaken for genuine mastery.

English 24/216 → **28/216**. 188 concepts remain. Next candidates
(topological level 9, to be confirmed fresh next batch): `eng.listening.
active-listening`, `eng.phonetics.rhythm-and-timing`, `eng.speaking.oral-
fluency`, `eng.vocab.word-recognition`. All five tracking files updated
in the same commit. Full validation: KG validator PASS (all 6 subjects,
English 216/216 reachable, unchanged), `npx tsc --noEmit` clean, existing
test suite green, no runtime/KG/Blueprint/Teaching Asset file touched.

### English — Batch 8: topological level 9 (2026-08-03, English Educational Brain authoring program)

Continuing directly after Batch 7. Recomputed the topological queue fresh
from the live KG — confirmed exactly 4 concepts at level 9:
`eng.listening.active-listening` (unlocked by `eng.phonetics.prosody`),
`eng.phonetics.rhythm-and-timing` and `eng.speaking.oral-fluency` (both
unlocked by `eng.phonetics.connected-speech`), `eng.vocab.word-
recognition` (unlocked by `eng.phonics.decoding-fluency`). All 4 had
existing, substantial Blueprints, reused by reference. This batch is
notable as the first to draw concepts from three domains new to this
program's authoring (`eng.listening.*`, `eng.speaking.*`, `eng.vocab.*`),
each with a genuinely distinct evidence profile from the phonics/
phonetics-domain concepts authored in Batches 1-7.

Notable findings: (1) `eng.listening.active-listening`'s evidence profile
(mixed audio and non-auditory, socially-observed behaviour) and
`eng.vocab.word-recognition`'s (self-report-dependent, since the tutor
cannot independently verify a learner's genuine prior vocabulary) are
both genuinely distinct from every phonetics-domain concept authored so
far, flagged for the Pipeline's awareness at future concepts in these
domains; (2) `eng.phonetics.rhythm-and-timing`'s first misconception was
identified as a rare case where the most likely birth type differs
systematically by learner L1 background (instruction-induced for most
learners, but overgeneralization-of-a-correct-L1-system for syllable-
timed-L1 learners); (3) `eng.speaking.oral-fluency`'s misconception pair
was confirmed as a second instance of the "premature perfectionism,
redirected" pattern already found at `eng.phonics.decoding-fluency`
(an initial too-strict standard, corrected, followed by a predictable
overcorrection to a different too-strict standard); (4) `eng.vocab.word-
recognition`'s certification standard was identified as distinctive in
this program — explicitly rewarding an honest "I don't know" over a
confident false claim of recognition, flagged as a template for any
future concept where face-saving false positives are a genuine threat to
accurate assessment.

English 28/216 → **32/216**. 184 concepts remain. Next candidates
(topological level 10, to be confirmed fresh next batch): `eng.grammar.
word-classes-overview`, `eng.listening.distinguishing-sounds-in-speech`,
`eng.listening.listening-for-gist`, `eng.phonetics.accents-and-dialects`,
`eng.speaking.pronunciation-in-conversation`, `eng.vocab.context-clues`,
`eng.vocab.synonyms-antonyms`, `eng.vocab.word-families`. All five
tracking files updated in the same commit. Full validation: KG validator
PASS (all 6 subjects, English 216/216 reachable, unchanged), `npx tsc
--noEmit` clean, existing test suite green, no runtime/KG/Blueprint/
Teaching Asset file touched.

### English — Batch 9 (part 1 of 2): topological level 10 (2026-08-03, English Educational Brain authoring program)

Continuing directly after Batch 8. Recomputed the topological queue fresh
from the live KG — confirmed exactly 8 concepts at level 10 (the largest
single level this program has authored so far), split into two parts of
4 per this program's established batch-size discipline. Part 1 covers:
`eng.grammar.word-classes-overview` (unlocked by `eng.vocab.word-
recognition`, this program's first `eng.grammar.*` domain entry),
`eng.listening.distinguishing-sounds-in-speech` (unlocked jointly by
`eng.phonetics.minimal-pairs` and `eng.listening.active-listening`, a
genuine two-prerequisite convergence node and a confirmed terminal
concept — `unlocks: []`), `eng.listening.listening-for-gist` (unlocked by
`eng.listening.active-listening`), and `eng.phonetics.accents-and-
dialects` (unlocked by `eng.phonetics.rhythm-and-timing`, crossing into a
new `eng.linguistics.*` domain via its own single unlock,
`eng.linguistics.sociolinguistics-intro`). All 4 had existing,
substantial Blueprints, reused by reference.

Notable findings: (1) `eng.listening.distinguishing-sounds-in-speech` is
this program's first concept with two direct prerequisites drawn from two
different domains (`eng.phonetics.*` and `eng.listening.*`) converging at
once, and its terminal (`unlocks: []`) status was flagged as warranting
unusually careful mastery-probe diagnostic precision, since no downstream
concept will naturally catch a partially-mastered gap; (2)
`eng.phonetics.accents-and-dialects` was identified as this program's most
affect-sensitive concept authored to date, exceeding even `eng.vocab.
word-recognition`'s honesty-as-mastery-criterion concern, given its
content directly touches personal/family accent identity — flagged as a
template for any future concept in similar territory; (3) `eng.grammar.
word-classes-overview`'s two misconceptions were both classified as Type 1
(overgeneralization) of otherwise-reasonable defaults (words mostly do
stay in one class; meaning mostly does predict class), a pattern
consistent with this program's general finding that foundational,
domain-opening concepts tend toward overgeneralization-type misconceptions
rather than the more specialized birth types; (4) `eng.listening.
listening-for-gist`'s two direct unlocks (`eng.listening.listening-for-
detail`, `eng.listening.listening-comprehension-strategies`) were recorded
as the program's next `eng.listening.*` priority once the domain's current
frontier is cleared.

English 32/216 → **36/216**. 180 concepts remain. Next candidates (level
10, part 2, to be confirmed fresh next batch): `eng.speaking.
pronunciation-in-conversation`, `eng.vocab.context-clues`, `eng.vocab.
synonyms-antonyms`, `eng.vocab.word-families`. All five tracking files
updated in the same commit. Full validation: KG validator PASS (all 6
subjects, English 216/216 reachable, unchanged), `npx tsc --noEmit`
clean, existing test suite green, no runtime/KG/Blueprint/Teaching Asset
file touched.

### English — Batch 9 (part 2 of 2): topological level 10 completion (2026-08-03, English Educational Brain authoring program)

Continuing directly after Batch 9 part 1, completing level 10's
remaining 4 concepts: `eng.vocab.context-clues`, `eng.vocab.synonyms-
antonyms`, `eng.vocab.word-families` (all three unlocked by `eng.vocab.
word-recognition`), and `eng.speaking.pronunciation-in-conversation`
(unlocked by `eng.speaking.oral-fluency`, cross-linked to `eng.phonetics.
prosody`). All 4 had existing, substantial Blueprints, reused by
reference.

Notable findings: (1) `eng.vocab.word-families`'s first misconception
(grouping by spelling similarity alone) was classified as this program's
first genuinely confirmed Type 4 (notation-induced) birth type — a
rarer category than the Type 1/Type 3 patterns dominating this program's
authoring so far, worth noting as confirmation the full birth-taxonomy
generalizes to real content, not just theory; (2) `eng.speaking.
pronunciation-in-conversation`'s second misconception (native-like-
accent-as-the-goal) was identified as a second, independent confirmation
of the accent/identity affect-sensitivity pattern first flagged at
`eng.phonetics.accents-and-dialects` (this same batch's part 1) — now
established as a recurring theme across at least two curriculum points,
not an isolated case; (3) `eng.vocab.context-clues`'s revision-readiness
certification standard was confirmed as a second instance of the honesty/
metacognitive-accuracy-as-mastery-criterion pattern first identified at
`eng.vocab.word-recognition`, strengthening the case that this is a
genuine, recurring `eng.vocab.*` domain design feature rather than a
one-off; (4) `eng.vocab.synonyms-antonyms` and `eng.vocab.word-families`
were both confirmed as clean examples of well-scoped unlock design — each
of their combined 5 direct unlocks has an unambiguous, single-purpose
relationship to its prerequisite, with no overlap or redundancy.

English 36/216 → **40/216**. Level 10 (8 concepts across both parts) is
now fully complete. 176 concepts remain. Next candidates (topological
level 11, to be confirmed fresh next batch) not yet computed this batch,
per this program's per-batch fresh-recomputation discipline — deferred to
the next batch's start. All five tracking files updated in the same
commit. Full validation: KG validator PASS (all 6 subjects, English
216/216 reachable, unchanged), `npx tsc --noEmit` clean, existing test
suite green (198 files, 4017 passed / 9 skipped), no runtime/KG/
Blueprint/Teaching Asset file touched.
