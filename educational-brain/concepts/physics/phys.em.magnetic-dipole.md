...
A bar magnet has a north pole and a south pole — it is a magnetic dipole. So is a current loop: current flowing in a circle creates a magnetic field that looks identical (at large distances) to a bar magnet's field, with a north end along the axis in one direction and a south end in the other. At the atomic level, the magnetic moments of electrons (from both their orbital motion and their intrinsic spin) are the microscopic source of all macroscopic magnetism.

The magnetic dipole moment is m = NIA — the product of the number of turns N, the current I, and the area of the loop A. The direction is given by the right-hand rule: curl the fingers in the direction of current flow, and the thumb points in the direction of the magnetic moment vector (toward the north end). Units: A·m² (ampere-metres squared).

In an external magnetic field B, a magnetic dipole behaves exactly as an electric dipole does in an electric field: it experiences a torque τ = m × B that tends to align the moment with the field, and in a non-uniform field, a net force that attracts or repels it depending on orientation.

Earth's magnetic field is generated by convective currents of molten iron in its outer core — effectively a giant current loop. The result is approximately a magnetic dipole field. But the geographic north pole (toward which compasses point) is actually a magnetic south pole — it attracts the compass needle's north-seeking end. Earth's magnetic poles are not aligned with the geographic poles (the angle between them is called magnetic declination, which varies by location and changes slowly over time as the core circulation evolves). Earth's poles have also reversed completely many times in geological history, recorded in the alignment of iron-rich minerals in ancient rock layers.
