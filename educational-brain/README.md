...
- **Delivery 15 — Evidence Architecture** (in-repo, 2026-07-10, Phase 2:
  from knowledge library to teaching instrument):
  `validation/08-evidence-architecture.md` + a quality-bar amendment in
  `concepts/COVERAGE.md`. Re-evaluates the whole tree against the
  First-Principles Review's standard ("does this generate proprietary
  educational evidence no general AI can possess?"): moat classification
  of every library; the unified per-component evidence contract
  (consolidating contracts already scattered across decision-engine/08
  §3, student-state/06 §7, decision-engine/05 law 3, first-lesson/08 —
  connection, not invention); eight missing evidence loops where the
  Brain teaches but learns nothing (L1 recovery efficacy never
  aggregates cross-learner; L2 anti-analogy lists don't grow from
  Type-6 birth events; L3 no per-concept decay norms; L4 the 27-action
  catalog has no cross-learner outcome table; L5 decisions never joined
  to next-turn consequences; L6 placement trust priors never retune
  from measured frontiers; L7 curriculum feedback is prose not events;
  L8 voice evidence forfeited — already documented, reframed); the
  authoring pivot — concept entries are now valid as INSTRUMENTED
  SKELETONS (misconceptions/probes/gates at seed quality, prose
  deferred), rich-prose-no-instruments remains invalid; and the
  million-student answer (misconception epidemiology, per-concept decay
  half-lives, the action effectiveness table, probe item science,
  calibrated placement priors, recovery science, the empirical
  curriculum audit — seven bodies of knowledge that can only be
  accumulated by the venue, never generated by a model). The
  entry-as-falsifiable-predictions reframe: every authored claim in a
  concept entry is a testable prediction that owes its test. Knowledge
