...
strict KG-prerequisite order through 161 concepts total (`educational-brain/concepts/biology/`,
no Blueprint exists for biology as a subject; 51 entries — see COVERAGE.md's biology row
...
Authored via 48 successive batches of 3, each recomputing the fresh topologically-ready
frontier from the prior baseline — see COVERAGE.md's biology row for the full per-batch
concept list; the latest batch (from the 158-concept baseline) added
`bio.behav.kin-selection-altruism`, `bio.behav.learning-and-behavior`, and
`bio.eco.predator-prey-dynamics` (all 3 first-principles-authored — 0 of 24 frontier
candidates had seed content, the FOURTEENTH consecutive fully zero-content batch; the
latter also cross-links to `math.de.ode`/`math.de.nonlinear-ode`) —
**EB is 161/199** (only 38 concepts remain).
**First-principles authoring remains the settled default mode** for continuing this
campaign from the current 199-concept KG frontier (fourteen consecutive zero-content
...
now been applied consistently across twenty batches (28 through 48) without a
