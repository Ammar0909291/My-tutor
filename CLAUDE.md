...
strict KG-prerequisite order through 185 concepts total (`educational-brain/concepts/biology/`,
no Blueprint exists for biology as a subject; 75 entries — see COVERAGE.md's biology row
...
Authored via 56 successive batches of 3, each recomputing the fresh topologically-ready
frontier from the prior baseline — see COVERAGE.md's biology row for the full per-batch
concept list; the latest batch (from the 182-concept baseline) added
`bio.eco.biogeochemistry-advanced`, `bio.eco.landscape-conservation-ecology`, and
`bio.evo.phylogeography-biogeography` (all 3 first-principles-authored — 0 of 14 frontier
candidates had seed content, the TWENTY-SECOND consecutive fully zero-content batch) —
**EB is 185/199** (only 14 concepts remain).
**First-principles authoring remains the settled default mode** for continuing this
campaign from the current 199-concept KG frontier (twenty-two consecutive zero-content
...
now been applied consistently across twenty-eight batches (28 through 56) without a
