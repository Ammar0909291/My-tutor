...
strict KG-prerequisite order through 167 concepts total (`educational-brain/concepts/biology/`,
no Blueprint exists for biology as a subject; 57 entries — see COVERAGE.md's biology row
...
Authored via 50 successive batches of 3, each recomputing the fresh topologically-ready
frontier from the prior baseline — see COVERAGE.md's biology row for the full per-batch
concept list; the latest batch (from the 164-concept baseline) added
`bio.behav.animal-cognition`, `bio.eco.global-change-biology`, and
`bio.eco.applied-ecology-ecosystem-services` (all 3 first-principles-authored — 0 of 20
frontier candidates had seed content, the SIXTEENTH consecutive fully zero-content batch;
this batch closes out `bio.behav` entirely — `bio.behav` and `bio.neuro` are now both fully
authored domains) —
**EB is 167/199** (only 32 concepts remain).
**First-principles authoring remains the settled default mode** for continuing this
campaign from the current 199-concept KG frontier (sixteen consecutive zero-content
...
now been applied consistently across twenty-two batches (28 through 50) without a
