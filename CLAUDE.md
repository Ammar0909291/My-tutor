...
Probe-depth Batch 1-13 (`biologyDepthSeedAssets.ts`, `bio.found` then `bio.cell` then `bio.mol`
then `bio.physio` then `bio.gen` then `bio.eco` then `bio.evo` then `bio.micro` then `bio.div`
then `bio.plant` then `bio.repro` then `bio.immuno` then `bio.sys`, 97 concepts) close the same
...
**97/108 at contract** (11 remain, 2/3 gradeable each). Formal EB
