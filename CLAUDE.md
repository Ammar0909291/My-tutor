...
strict KG-prerequisite order through 53 concepts total (`educational-brain/concepts/biology/`,
no Blueprint exists for biology as a subject; 2 entries, `scientific-method-in-biology` and
`unifying-themes-in-biology`, are for ZERO-seed-content concepts; five entries,
`bio.eco.nutrient-cycling`, `bio.eco.community-ecology`, `bio.physio.respiratory-system`,
`bio.physio.circulatory-system`, and `bio.plant.photosynthesis`, record genuine KG-description-
vs-seed-content gaps as Curriculum Feedback rather than fabricating content). Authored via 12
successive batches of 3, each recomputing the fresh topologically-ready frontier from the prior
baseline — see COVERAGE.md's biology row for the full per-batch concept list; the latest batch
(from the 50-concept baseline) added `bio.mol.translation-genetic-code`,
`bio.physio.circulatory-system`, `bio.plant.photosynthesis` — **EB is 53/199**. These two
