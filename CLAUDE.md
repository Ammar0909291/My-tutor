...
strict KG-prerequisite order through 74 concepts total (`educational-brain/concepts/biology/`,
no Blueprint exists for biology as a subject; 2 entries, `scientific-method-in-biology` and
`unifying-themes-in-biology`, are for ZERO-seed-content concepts; sixteen entries — see
COVERAGE.md's biology row for the full list — record genuine KG-description-vs-seed-content
gaps as Curriculum Feedback rather than fabricating content). Authored via 19 successive batches
...
71-concept baseline) added `bio.micro.viral-replication`, `bio.cell.cell-signalling`,
`bio.cell.apoptosis` — **EB is 74/199**. These two
