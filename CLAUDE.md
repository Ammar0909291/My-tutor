...
strict KG-prerequisite order through 44 concepts total (`educational-brain/concepts/biology/`,
...
Feedback rather than fabricating content). Authored via 9 successive batches of 3, each
recomputing the fresh topologically-ready frontier from the prior baseline — see COVERAGE.md's
biology row for the full per-batch concept list; the latest batch (from the 41-concept baseline)
added `bio.gen.pedigree-human-genetics`, `bio.mol.nucleic-acid-structure`,
`bio.physio.digestive-system` — **EB is 44/199**. These two
