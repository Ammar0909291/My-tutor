...
strict KG-prerequisite order through 152 concepts total (`educational-brain/concepts/biology/`,
no Blueprint exists for biology as a subject; 44 entries — see COVERAGE.md's biology row
...
Authored via 45 successive batches of 3, each recomputing the fresh topologically-ready
frontier from the prior baseline — see COVERAGE.md's biology row for the full per-batch
concept list; the latest batch (from the 149-concept baseline) added
`bio.behav.animal-communication`, `bio.behav.foraging-behavior`, and
`bio.neuro.neurodevelopment` (all 3 first-principles-authored — 0 of 24 frontier
candidates had seed content, the ELEVENTH consecutive fully zero-content batch) —
**EB is 152/199** (only 47 concepts remain).
**First-principles authoring remains the settled default mode** for continuing this
campaign from the current 199-concept KG frontier (eleven consecutive zero-content
...
now been applied consistently across seventeen batches (28 through 45) without a
