...
Probe-depth Batch 1-10 (`biologyDepthSeedAssets.ts`, `bio.found` then `bio.cell` then `bio.mol`
then `bio.physio` then `bio.gen` then `bio.eco` then `bio.evo` then `bio.micro` then `bio.div`
then `bio.plant`, 83 concepts) close the same zero-slack defect physics/chemistry already had:
every one of biology's 108 authored concepts was stuck at 2/3 gradeable probes, so no biology
lesson could ever reach verified mastery — now **83/108 at contract** (25 remain, 2/3 gradeable
each). Formal EB
