...
Probe-depth Batch 1-12 (`biologyDepthSeedAssets.ts`, `bio.found` then `bio.cell` then `bio.mol`
then `bio.physio` then `bio.gen` then `bio.eco` then `bio.evo` then `bio.micro` then `bio.div`
then `bio.plant` then `bio.repro` then `bio.immuno`, 93 concepts) close the same zero-slack
defect physics/chemistry already had: every one of biology's 108 authored concepts was stuck
at 2/3 gradeable probes, so no biology lesson could ever reach verified mastery — now
**93/108 at contract** (15 remain, 2/3 gradeable each). Formal EB
...
Feedback rather than fabricating content); the latest 3 — `bio.evo.evidence-for-evolution`,
`bio.mol.enzymes`, `bio.cell.mitosis` — are a fourth recomputed topological frontier after the
26-concept baseline) — **EB is 29/199**. These two
