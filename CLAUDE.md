...
strict KG-prerequisite order through 164 concepts total (`educational-brain/concepts/biology/`,
no Blueprint exists for biology as a subject; 54 entries — see COVERAGE.md's biology row
...
Authored via 49 successive batches of 3, each recomputing the fresh topologically-ready
frontier from the prior baseline — see COVERAGE.md's biology row for the full per-batch
concept list; the latest batch (from the 161-concept baseline) added
`bio.behav.human-behavioral-ecology-evolutionary-psych`,
`bio.neuro.cognitive-neuroscience-consciousness`, and `bio.neuro.neurodegenerative-disease`
(all 3 first-principles-authored — 0 of 22 frontier candidates had seed content, the
FIFTEENTH consecutive fully zero-content batch; this batch closes out `bio.neuro` entirely,
leaving only `bio.behav.animal-cognition` remaining in `bio.behav`) —
**EB is 164/199** (only 35 concepts remain).
**First-principles authoring remains the settled default mode** for continuing this
campaign from the current 199-concept KG frontier (fifteen consecutive zero-content
...
now been applied consistently across twenty-one batches (28 through 49) without a
