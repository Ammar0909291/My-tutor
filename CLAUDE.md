...
Probe-depth Batch 1-5 (`biologyDepthSeedAssets.ts`, `bio.found` then `bio.cell` then `bio.mol`
then `bio.physio` then `bio.gen`, 52 concepts) close the same zero-slack defect physics/chemistry
already had: every one of biology's 108 authored concepts was stuck at 2/3 gradeable probes, so
no biology lesson could ever reach verified mastery — now **52/108 at contract** (56 remain, 2/3
gradeable each). Formal EB
